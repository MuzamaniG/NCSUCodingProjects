...
public class AdjacencyMatrixGraph<V, E> extends EdgeListGraph<V, E> {
    ...
    public Edge<E> insertEdge(Vertex<V> vertex1, Vertex<V> vertex2, E edgeData) {
    	MatrixVertex v1 = validate(vertex1);
        MatrixVertex v2 = validate(vertex2);
        int oneIndex = v1.getIndex();
        int twoIndex = v2.getIndex();
        Edge<E> newEdge = super.insertEdge(vertex1, vertex2, edgeData);
        matrix[oneIndex][twoIndex] = (GraphEdge) newEdge;
        if (!isDirected()) {
            matrix[twoIndex][oneIndex] = (GraphEdge) newEdge;
        }
        return newEdge;
    }
    ...
    public void removeEdge(Edge<E> edge) {
        ...
        super.removeEdge(e);
        MatrixVertex v1 = (MatrixVertex) e.getEndpoints()[0];
        MatrixVertex v2 = (MatrixVertex) e.getEndpoints()[1];
