...
public class AdjacencyListGraph<V, E> extends EdgeListGraph<V, E> {
    ...
    public Edge<E> insertEdge(Vertex<V> vertex1, Vertex<V> vertex2, E edgeData) {
    	...
    	Iterable<Edge<E>> edges = outgoingEdges(vertex1);
    	Iterable<Edge<E>> edges2 = incomingEdges(vertex2);
    	Position<Edge<E>> position = ((PositionalList<Edge<E>>) edges).addLast(e);
    	ALEdge edge = validate(e);
    	edge.setOutgoingListPosition(position);
    	position = ((PositionalList<Edge<E>>) edges2).addLast(e);
    	edge.setIncomingListPosition(position);
    	return edge;
    }
     ...
     * @param v vertex being validated 
