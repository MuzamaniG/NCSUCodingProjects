...
public class SkipListMap<K extends Comparable<K>, V> extends AbstractOrderedMap<K, V> {
    ...
    public V put(K key, V value) {
		...
		do {
			...
			newNode = insertAfterAbove(temp, newNode, new MapEntry<>(key, value));
    ...
    public V remove(K key) {
    	   SkipListNode<K, V> node = lookUp(key);
    	    if (node.getEntry() == null || !node.getEntry().getKey().equals(key)) {
    	        return null; 
    	    }
    	    V removedValue = node.getEntry().getValue();
    	    SkipListNode<K, V> toRemove = node;
    	    while (toRemove != null) {
    	        ...
    	        toRemove = toRemove.getAbove();
    	    }
    	    size--;
    	    while (height > 0 && start.getNext() == start.getNext().below) {
    	        start = start.below;
    	        height--;
    	    }
    	    return removedValue;
